...
if(base64_decode($signature) == base64_decode($valid)){
        ...
        if(!$nulo){
                ...
                $query = $query."(febre, fadiga, tosse_seca, tosse_persistente, espirros, coriza, dor_garganta, diarreia, dor_cabeca, falta_ar, dor_corpo, emagrecimento, desidratacao, falta_apetite, sincope, confusao_mental, sonolencia_excessiva, irritabilidade, perda_paladar, perda_olfato, sudorese, vomito, dia, outro, pontos_sintoma)";
                $query = $query." values ";
                $query = $query."(:febre, :fadiga, :tosse_seca, :tosse_persistente, :espirros, :coriza, :dor_garganta, :diarreia, :dor_cabeca, :falta_ar, :dor_corpo, :emagrecimento, :desidratacao, :falta_apetite, :sincope, :confusao_mental, :sonolencia_excessiva, :irritabilidade, :perda_paladar, :perda_olfato, :sudorese, :vomito, :dia, :outro, :pontos_sintoma);";
                ...
                $pontos = 8*((int) $post->febre+(int) $post->fadiga+(int) $post->tosse_seca+(int) $post->tosse_persistente+(int) $post->espirros+(int) $post->coriza+(int) $post->dor_garganta+(int) $post->diarreia+(int) $post->dor_cabeca+(int) $post->falta_ar);
                $pontos += 8*((int) $post->dor_corpo+(int) $post->emagrecimento+(int) $post->desidratacao+(int) $post->falta_apetite+(int) $post->sincope+(int) $post->confusao_mental+(int) $post->sonolencia_excessiva+(int) $post->irritabilidade);
                $pontos += 8*((int) $post->perda_paladar+(int) $post->perda_olfato+(int) $post->sudorese+(int) $post->vomito);
                $insere->bindValue(":pontos_sintoma",$pontos,PDO::PARAM_BOOL);
                ...
                if($foi){
                        ...
                        //
                        $query3 = "select pontos__doenca from usuario where id_usuario=:id_usuario"; 
                        $insere3 = $pdo->prepare($query3);
                        $insere3->bindValue(":id_usuario",$id_usuario,PDO::PARAM_INT);
                        $foi3 = $insere3->execute();
                
                        $result3 = $insere3->fetchAll();
                        $pontos_doenca = $result3[0]["pontos_doenca"];
                        //                        
                               
                        $soma_pontos = $pontos + $pontos_doenca;
                        
                        echo $soma_pontos;
                        
                        $query2 = "insert into usuario_in_dia"; 
                        $query2 = $query2."(usuario_id_usuario, dia_id_dia, soma_pontos)";
                        ...
                        $insere2->bindValue(":soma_pontos",$soma_pontos,PDO::PARAM_INT);
